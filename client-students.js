export class StudentManager {
        ...
        try {
            if (isEdit) {
                ...
                if (response.ok) {
                    ...
                    await window.customAlert('שגיאה בעדכון: ' + (err.message || err.error || 'לא ידועה'), true);
                }
            } else {
                ...
                if (!autoCode) {
                    ...
                    if (!code) {
                        await window.customAlert('יש להזין קוד תלמיד רצוי או לסמן "הקצה קוד תלמיד עוקב אוטומטית".', true);
                ...
                if (response.ok) {
                    ...
                    await window.customAlert(`התלמיד נוצר בהצלחה! קוד התלמיד במערכת הוא: ${result.student_code}`);
                    document.getElementById('studentFormModal').classList.add('hidden');
                    if (this.onRefreshCallback) this.onRefreshCallback();
                } else {
                    const err = await response.json().catch(() => ({}));
                    if (response.status === 400 && err.error && err.error.includes("exists")) {
                        await window.customAlert('שגיאה: קוד התלמיד כבר קיים במערכת. בחר קוד אחר.', true);
                    } else {
                        await window.customAlert('שגיאה ביצירה: ' + (err.message || err.error || 'לא ידועה'), true);
                    }
                }
            }
        } catch (error) {
            await window.customAlert('שגיאת תקשורת במערכת בעת השמירה.', true);
        } finally {
            submitBtn.disabled = false;
            submitBtn.innerHTML = origText;
        }
    ...
    async changeStudentCode(oldCode) {
        const newCode = await window.customPrompt(`הזן את קוד התלמיד החדש שיחליף את הקוד הקודם (${oldCode}):`);
        if (newCode === null || newCode.trim() === '') return;
        
        const finalNewCode = newCode.trim();
        if (finalNewCode === oldCode) {
            await window.customAlert('הקוד החדש זהה לקוד הישן. לא בוצע שינוי.', true);
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                await window.customAlert(`מעולה! קוד התלמיד עודכן בהצלחה ל-${finalNewCode} בכל רישומי המערכת.`);
                document.getElementById('studentFormModal').classList.add('hidden');
                if (this.onRefreshCallback) this.onRefreshCallback();
            } else {
                const err = await response.json().catch(() => ({}));
                if (response.status === 409 || (err.error && err.error.includes('exists'))) {
                    await window.customAlert('שגיאה: קוד התלמיד החדש שבחרת כבר תפוס על ידי תלמיד אחר במערכת.', true);
                } else {
                    await window.customAlert('אירעה שגיאה בשינוי הקוד: ' + (err.message || err.error || 'נסה שוב מאוחר יותר.'), true);
                }
            }
        } catch (error) {
            await window.customAlert('שגיאת תקשורת במערכת. פעולת שינוי הקוד לא בוצעה.', true);
        }
    }
